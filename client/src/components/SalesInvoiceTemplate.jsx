...
const SalesInvoiceTemplate = forwardRef(({ order, handlePrint, title, type }, ref) => {
      ...
      <Paper p="xl" radius="md" shadow="sm" style={{ maxWidth: 1100, margin: "auto" }}>
        ...
        <Grid mb="md">
          <Grid.Col span={6}>
            <Stack gap={4}>
              <Text size="sm">
                <strong>{t("Supplier")}:</strong> {order.client?.fullName}
              </Text>
              <Text size="sm">
          ...
          <Grid.Col span={6}>
            <Stack gap={4} align="flex-end">
              <Text size="sm">
                <strong>{t("Invoice Total Amount")}:</strong> {order?.totalSalePrice} SDG
              </Text>
              <Text size="sm">
                <strong>{t("Paid Amount")}:</strong> {formatCurrency(order?.paidAmount)} SDG
              </Text>
              <Text size="sm">
                <strong>{t("Remaining")}:</strong> {formatCurrency(order?.remainingAmount.toLocaleString())} SDG
              </Text>
            </Stack>
          </Grid.Col>
        </Grid>
          ...
          <tbody>
            {order.products?.map((item, idx) => (
              <tr key={idx} style={{ textAlign: "center" }}>
                ...
                <td>{item?.quantity}</td>
                <td>{item?.unit}</td>
                <td>{formatCurrency(item?.productSnapShot?.salePrice.toLocaleString())}</td>
                <td>{formatCurrency(item?.products?.unitTotalPrice?.toLocaleString())}</td>
              </tr>
            ))}
          </tbody>
        </Table>
            ...
            {order.payments?.length > 0 ? (
                order.payments.map((payment, paymentIndex) =>
                    ...
                    >
                        ...
                        <td>{formatCurrency(transaction?.amount).toLocaleString()}</td>

                        <td>
                        {transaction.method === "Bankak"
